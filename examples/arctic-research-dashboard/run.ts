...
// --- E2B: resize for dashboard ---

const postProcess = (imageUrl: string) =>
  Effect.gen(function* () {
    yield* Console.log("[6/7] E2B sandbox: downloading + resizing for dashboard...");
...
urllib.request.urlretrieve("${imageUrl}", "/tmp/dashboard-hero.png")
import os
size = os.path.getsize("/tmp/dashboard-hero.png")
print(f"Downloaded: {size} bytes, ready for dashboard integration")
`,
        );
