...
const motionVocab = (implementation?.motion_vocabulary ?? {}) as Record<string, Record<string, unknown>>
...
function paletteToTokens(paletteType: string, styleId: string): Record<string, string> {
  const palettes: Record<string, Record<string, string>> = {
    "jewel tones with gold and black accents": {
      primary: "#C9A84C", secondary: "#1A1A1A", accent: "#8B0000",
      surface: "#0D0D0D", "on-surface": "#F5F0E8", muted: "#2A2420",
    },
    "soft pastels with warm highlights and cool shadows": {
      primary: "#E8B4B8", secondary: "#A8C4D8", accent: "#F0D58C",
      surface: "#FDF8F4", "on-surface": "#3D3835", muted: "#D4C5BE",
    },
    "red, black, white, occasional ochre": {
      primary: "#CC0000", secondary: "#000000", accent: "#CC8800",
      surface: "#FFFFFF", "on-surface": "#000000", muted: "#666666",
    },
    default: {
      primary: "#2563EB", secondary: "#1E293B", accent: "#F59E0B",
      surface: "#FFFFFF", "on-surface": "#111111", muted: "#64748B",
    },
  }
...
async function generateFigmaTokenSets() {
  ...
  for (const style of styles) {
    ...
    const fontSelection = (impl.typography ?? profile.font_selection ?? {}) as Record<string, string>
    ...
    const tokenSet = {
      ...
      typography: {
        display: {
          fontFamily: { value: fontSelection.display ?? style.dsp.typography_family ?? "system-ui", type: "fontFamilies" },
        },
        body: {
          fontFamily: { value: fontSelection.body ?? style.dsp.typography_family ?? "system-ui", type: "fontFamilies" },
        },
      },
      spacing: {
...
const tailwindCount = (await Array.fromAsync(new Bun.Glob("*.css").scan(`${SKILL_DIR}/assets/tailwind-presets`))).length
const motionCount = (await Array.fromAsync(new Bun.Glob("*.json").scan(`${SKILL_DIR}/assets/motion-presets`))).length
const figmaCount = (await Array.fromAsync(new Bun.Glob("*.json").scan(`${SKILL_DIR}/assets/figma-token-sets`))).length
