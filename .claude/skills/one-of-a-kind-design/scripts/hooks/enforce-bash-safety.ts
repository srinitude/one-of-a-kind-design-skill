...
const RUNTIME_PATTERNS: readonly SafetyPattern[] = [
  ...
  {
    regex: /\bnpm\s+(install|run|test|exec|start|build|ci)\b/,
    name: "npm",
...
const DESTRUCTIVE_PATTERNS: readonly SafetyPattern[] = [
  ...
  {
    regex: /\brm\b[^|]*-[^\s]*[rR][^\s]*\s+\/(\s|$|\*)/,
    name: "rm -r /",
    reason: "Recursive delete of root blocked.",
  },
  {
    regex: /\brm\b[^|]*-[^\s]*[rR][^\s]*\s+\.(\s|$|[;&|])/,
    name: "rm -r .",
    reason: "Recursive delete of cwd blocked.",
  },
  {
    regex: /\brm\b[^|]*-[^\s]*[rR][^\s]*\s+~(\s|$|[;&|]|\/\*)/,
    name: "rm -r ~",
    reason: "Recursive delete of home blocked.",
  },
...
const GIT_DANGER_PATTERNS: readonly SafetyPattern[] = [
  ...
  },
  {
    ...
    name: "git reset --hard",
    reason: "Hard reset blocked. Use git stash or soft reset.",
  },
  {
    regex: /\bgit\s+clean\s+-[^\s]*f[^\s]*d/,
    name: "git clean -fd",
    reason: "Forced clean blocked. Review untracked files first.",
  },
  {
    regex: /\bgit\s+branch\s+-D/,
    name: "git branch -D",
    reason: "Force branch delete blocked. Use -d for safe delete.",
  },
];
...
const INJECTION_PATTERNS: readonly SafetyPattern[] = [
  {
    regex: /\b(curl|wget)\s+.*\|\s*(bash|sh)\b/,
    name: "pipe to shell",
